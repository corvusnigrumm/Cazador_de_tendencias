"""
TrendRadar SEO — trends/scorer.py
===================================
...
from trendradar.config import SCORE_WEIGHTS, DISCOVER_THRESHOLD
...
class TrendScorer:
    ...
    def __init__(self, weights: Optional[dict] = None, threshold: Optional[int] = None):
        ...
        self.threshold = threshold or DISCOVER_THRESHOLD
    ...
    def _score_one(self, topic: TrendingTopic) -> "ScoredTopic":
        ...
        scored.trend_score = round(
            scored.score_peak     * w.get("interest_peak",   0.35)
            + scored.score_velocity * w.get("growth_velocity", 0.30)
            + scored.score_curve    * w.get("recency",         0.15)
            + scored.score_momentum * w.get("news_coverage",   0.20),
            1,
        )
