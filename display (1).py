...
# ──────────────────────────────────────────────
# Tabla de ScoredTopics (con TrendScore y Discover)
# ──────────────────────────────────────────────
def print_scored_table(scored_topics: list, geo: str, timeframe_label: str) -> None:
    """
    Renderiza la tabla principal de ScoredTopics con TrendScore,
    tier de Discover y recomendación editorial.

    Columnas:
      # | Score | Tier | Tema | Curva | Peak | Ahora | Crecimiento | Recomendación
    """
    from trendradar.trends.fetcher import _sparkline
...
def print_scored_detail(s) -> None:
    """
    Detalle expandido de un ScoredTopic: score por dimensión,
    señales Discover activas y recomendación completa.
    """
    from trendradar.trends.fetcher import _sparkline
    ...
    if s.raw_interest:
        ...
        console.print(f"  [dim]{len(s.raw_interest)} puntos · {s.timeframe_label}[/dim]")
        console.print()
